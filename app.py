...
def admin():
    ...
    education = Education.query.all()
    profile = Profile.query.first()
    return render_template('admin.html', projects=projects, skills=skills, education=education, profile=profile)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Simple migration to add about_image_url if it doesn't exist
        try:
            db.session.execute(db.text('ALTER TABLE profile ADD COLUMN about_image_url VARCHAR(500)'))
            db.session.commit()
        except:
            db.session.rollback()
